...
num_randarray = length(num_antennas);
arrays = zeros(N,N,length(num_antennas));

%{
for idx_antennas = 1:num_randarray
    array = zeros(N);
    array(randperm(N^2, num_antennas(idx_antennas))) = 1;
    arrays(:,:,idx_antennas) = array;
end
array_name = 'Rand';
%}
...
sigma = 10;
for idx_antennas = 1:num_randarray
    arrays(:,:,idx_antennas) = Gaussianarray_gen(N,num_antennas(idx_antennas),sigma);
end
...
for idx_array = 1:num_randarray     %アンテナ数を切り替えてループ
    ...
    for seed = 1:num_phase_bias 
        ...
        %結果を保存
        save_dir = sprintf('./figures14/M2_%s_antn%d_%s_N%d_K%d_sup%d_noise%d/',array_name,num_antennas(idx_array),obj_name,N,K,sup_size,noiseLv);
        mkdir(save_dir);
        ...
        finishMessage = sprintf('アンテナ数=%d (%d/%d),バイアスseed(%d/%d)の結果を保存 (RMSE_o_best=%.4f, RMSE_r_best=%.4f, [row,col]_add=[%d,%d])',num_antennas(idx_array),idx_array,num_randarray,seed,num_phase_bias,RMSE_o_best,RMSE_r_best,row_add_best,col_add_best);
        disp(finishMessage);
