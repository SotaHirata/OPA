...
num_antenna = N;
sigma = 7.5;!
array_name = sprintf('GaussAntn%dSigma%.1f',num_antenna,sigma);

%Gaussianアレイのパターン数
num_Gauss_trial = 1;
%Gaussianアレイをnum_Gauss_trialパターン用意
arrays = zeros(N,N,num_Gauss_trial);
...
for idx_K = 1:Ks_len %Kごとに最適なrho_Oを決める
    ...
    %parfor idx_rho = 1:num_rho_Os %rho_Oを切り替えて最適なrho_Oを探索
        ...
        %進捗を表示
        progress = sprintf('K=%d(%d/%d),rho_o=%.2e(%d/%d)のとき,RMSE:o_abs=%.4f,o_angle=%.4f,o_total=%.4f',K,idx_K,Ks_len,rho_O,idx_rho,num_rho_Os,RMSE_o_abs_best,RMSE_o_angle_best,RMSE_o_best);
        disp(progress);
...
%振幅と位相それぞれのRMSE
RMSEs_o_abs = zeros(Ks_len, 1);
stds_rmse_o_abs = zeros(Ks_len, 1);
RMSEs_o_angle = zeros(Ks_len, 1);
stds_rmse_o_angle = zeros(Ks_len, 1);
%位相バイアスのRMSE
...
for idx_K = 1:Ks_len     %Kを切り替えてループ
    ...
    for seed = 1:num_Gauss_trial
        ...
        %parfor trial = 1:num_inits
            %進捗を表示
            progress = sprintf('K=%d (%d/%d),Gaussアレイseed(%d/%d),初期値trial(%d/%d)を計算中',K,idx_K,Ks_len,seed,num_Gauss_trial,trial,num_inits);
            disp(progress);
        ...
        %結果を保存
        save_dir = sprintf('./figure_M1_検証/GaussianK/M1_%s_%s_N%d_K%d_sup%d_noise%d/',array_name,obj_name,N,K,sup_size,noiseLv);
        mkdir(save_dir);
        ...
        finishMessage = sprintf('K=%d(%d/%d),Gaussアレイseed(%d/%d)の結果を保存, RMSE:o_abs=%.4f,o_angle=%.4f,o_total=%.4f,r=%.4f, [row,col]_add=[%d,%d])',K,idx_K,Ks_len,seed,num_Gauss_trial,RMSE_o_abs_best,RMSE_o_angle_best,RMSE_o_best,RMSE_r_best,row_add_best,col_add_best);
        disp(finishMessage);
    ...
    RMSEs_o_abs(idx_K) = mean(RMSE_tmp_o_abs);
    stds_rmse_o_abs(idx_K) = std(RMSE_tmp_o_abs);
    RMSEs_o_angle(idx_K) = mean(RMSE_tmp_o_angle);
    stds_rmse_o_angle(idx_K) = std(RMSE_tmp_o_angle);
    RMSEs_r(idx_K) = mean(RMSE_tmp_r);
    stds_r(idx_K) = std(RMSE_tmp_r);

end

RMSE_path = './figure_M1_検証/GaussianK/RMSE/';
mkdir(RMSE_path);
