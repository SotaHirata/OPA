...
%TVのパラメタ
%rho_O = 0; %TVなし
rho_O = 2e6; %SNR=60,40の成功値（アンテナ数=100）
%rho_O = 1e7;
tv_th = 1e-2;%SNR=60,40の成功値（アンテナ数=100）
tv_tau = 0.05;
tv_iter = 5; %TVの反復数
...
stds_o = zeros(num_antennas_len, 1);
RMSEs_r = zeros(num_antennas_len, 1);
stds_r = zeros(num_antennas_len, 1);
...
for idx_antenna = 1:num_antennas_len     %アンテナ数を切り替えてループ
    ...
    for seed = 1:num_Gauss_trial
        ...
        %array = MyRect(N,ceil(sqrt(num_antenna)));
        ...
        %結果を保存
        save_dir = sprintf('./figure_Gaussianアレイ_検証/M2_%s_antn%d_%s_N%d_K%d_sup%d_noise%d/',array_name,num_antenna,obj_name,N,K,sup_size,noiseLv);
        mkdir(save_dir);
        ...
        finishMessage = sprintf('アンテナ数=%d (%d/%d),バイアスseed(%d/%d)の結果を保存 (RMSE_o_best=%.4f, RMSE_r_best=%.4f, [row,col]_add=[%d,%d])',num_antenna,idx_antenna,num_antennas_len,seed,num_Gauss_trial,RMSE_o_best,RMSE_r_best,row_add_best,col_add_best);
        disp(finishMessage);
    ...
    RMSEs_o(idx_antenna) = mean(RMSE_tmp_o);
    stds_o(idx_antenna) = std(RMSE_tmp_o);
    RMSEs_r(idx_antenna) = mean(RMSE_tmp_r);
    stds_r(idx_antenna) = std(RMSE_tmp_r);

end

RMSE_path = './figure_Gaussianアレイ_検証/RMSE/';
mkdir(RMSE_path);

figure(1000);
subplot(1,2,1)
errorbar(num_antennas,RMSEs_o,stds_o);
title('RMSE of object');
xlabel('Number of antennas');
ylabel('Reconstruction RMSE');

subplot(1,2,2)
errorbar(num_antennas,RMSEs_r,stds_r);
