...
for idx_sigma = 1:sigmas_len     %シグマを切り替えてループ
    ...
    for seed = 1:num_Gauss_trial
        ...
        %結果を保存
        save_dir = sprintf('./figure_GaussianSigma_検証/M2_%s_sigma%.1f_%s_N%d_K%d_sup%d_noise%d/',array_name,sigma,obj_name,N,K,sup_size,noiseLv);
        mkdir(save_dir);
...
RMSE_path = './figure_GaussianSigma_検証/RMSE/';
mkdir(RMSE_path);
