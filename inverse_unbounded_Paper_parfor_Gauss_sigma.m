...
for idx_sigma = 1:sigmas_len     %シグマを切り替えてループ
    ...
    for seed = 1:num_Gauss_trial
        ...
        %結果を保存
        save_dir = sprintf('./figure_GaussianSigma/M2_%s_sigma%.1f_%s_N%d_K%d_sup%d_noise%d/',array_name,sigma,obj_name,N,K,sup_size,noiseLv);
        mkdir(save_dir);
        ...
        finishMessage = sprintf('sigma=%.1f (%d/%d),バイアスseed(%d/%d)の結果を保存 (RMSE_o_best=%.4f, RMSE_r_best=%.4f, [row,col]_add=[%d,%d])',sigma,idx_sigma,sigmas_len,seed,num_Gauss_trial,RMSE_o_best,RMSE_r_best,row_add_best,col_add_best);
        disp(finishMessage);
    ...
    stds_o(idx_sigma) = std(RMSE_tmp_o);
    RMSEs_r(idx_sigma) = mean(RMSE_tmp_r);
    stds_r(idx_sigma) = std(RMSE_tmp_r);

end

RMSE_path = './figure_GaussianSigma/RMSE/';
mkdir(RMSE_path);

figure(1000);
subplot(1,2,1)
errorbar(sigmas,RMSEs_o,stds_o);
