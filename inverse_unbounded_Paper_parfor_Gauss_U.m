...
%アンテナ1つのFFP
Usigmas = [N/4,N/3.5,N/3,N/2.5,N/2,N,N*1.5,2*N];
Us_len = length(Usigmas);
Us = zeros(N,N,Us_len);
for idx_U = 1:Us_len
    Usigma = Usigmas(idx_U);
    U = MyGaussian2D(N,N,N/2+1,N/2+1,Usigma,Usigma);
    Us(:,:,idx_U) = U/max(U(:));
end
...
%アンテナ配置をnum_arrays通り用意パターンの設定
num_arrays = 10;
arrays = zeros(N,N,num_arrays);
for seed = 1:num_arrays
    arrays(:,:,seed) = Gaussianarray_gen(N,num_antenna,sigma);
end
...
for idx_U = 1:Us_len %Kごとに最適なrho_Oを決める
    %Kを設定
    K = Usigmas(idx_U);
    data_indice = randperm(K); %ミニバッチ列を用意
    ...
    for idx_rho = 1:num_rho_Os
        ...
        %進捗を表示
        progress = sprintf('K=%d (%d/%d),rho_o=%.2e(%d/%d)のとき:RMSE_o=%.4f',K,idx_U,Us_len,rho_O,idx_rho,num_rho_Os,RMSE_o_best);
        disp(progress);

    end %rho_Oを切り替えてのループ終わり

    [min_RMSE, min_idx_rho] = min(RMSE_TVs);
    rho_O_tuned(idx_U) = rho_Os(min_idx_rho);

    %進捗を表示
    progress = sprintf('K=%d (%d/%d)のチューニング結果: rho_O=%.2e,RMSE_o=%.4f',K,idx_U,Us_len,rho_Os(min_idx_rho),min_RMSE);
    disp(progress);
...
%% ハイパーパラメータ決定後の検証
rho_O_tuned = [rho_O];
%SSIM（被写体）RMSE（位相バイアス）,shiftdistグラフ描画用
RMSEs_o = zeros(Us_len, 1);
stds_rmse_o = zeros(Us_len, 1);
SSIMs_o = zeros(Us_len, 1);
stds_ssim_o = zeros(Us_len, 1);
RMSEs_r = zeros(Us_len, 1);
stds_r = zeros(Us_len, 1);
shift_dists = zeros(Us_len, 1);
stds_shiftdist = zeros(Us_len, 1);

for idx_U = 1:Us_len   %Uを切り替えてループ
    U = Us(:,:,idx_U);
    %ハイパーパラメータを設定
    %rho_O = rho_O_tuned(idx_U);

    %SSIM,RMSEの平均・標準偏差計算のための記憶用変数の初期化
    RMSE_tmp_o = zeros(num_arrays, 1); 
    SSIM_tmp_o = zeros(num_arrays, 1); 
    RMSE_tmp_r = zeros(num_arrays, 1); 
    shift_dist_tmp = zeros(num_arrays, 1); 

    %アンテナ配置seedをnum_arrays通り切り替えてループ
    for seed = 1:num_arrays
        %アレイを設定
        array = arrays(:,:,seed);
        %位相シフトKパターン（N×N×K）を設定
        ...
        for batch_start = 1:batch_size:K
            %batch_F = MyFFT2(A(:,:,batch_start:min(batch_start+batch_size-1, K)).*gpuArray(double(exp(1i*r))));
            batch_F = MyFFT2(A(:,:,batch_start:min(batch_start+batch_size-1, K)).*exp(1i*r)).*U;
            S(batch_start:min(batch_start+batch_size -1, K)) = sum(abs(batch_F).^2.*obj.*sup, [1,2]);
        end
        S = reshape(S, [K,1]);
        %S = awgn(S,noiseLv,'measured');
        ...
        parfor trial = 1:num_inits
            %進捗を表示
            progress = sprintf('Usigma=%.1f(%d/%d),アンテナ配置seed(%d/%d),初期値trial(%d/%d)を計算中',Usigmas(idx_U),idx_U,Us_len,seed,num_arrays,trial,num_inits);
            disp(progress);
            
            %figure(trial);
            O_hat = rand(N); %O_hatの初期値
            r_hat = array.*rand(N)*2*pi; %r_hatの初期値
            batch_es = zeros(max_itr,1);
            ...
            for epoch = 1:num_epoch
                for batch_start = 1:batch_size:K %ミニバッチ
                    ...
                    batch_idx = data_indice(batch_start:min(batch_start+batch_size-1, K));
                    batch_A = A(:,:,batch_idx);
                    batch_F = MyFFT2(batch_A.*exp(1i*r_hat)).*U;
                    batch_S_hat = reshape(sum(abs(batch_F).^2.*O_hat, [1,2]), [length(batch_idx),1]);
                    batch_e = batch_S_hat - S(batch_idx);
                    batch_es(itr) = mean(abs(batch_e).^2, 'all');
                    ...
                    %O,rの勾配
                    st_O = 2*sum(abs(batch_F).^2.*reshape(batch_e, [1,1,length(batch_idx)]), 3) + 2.*rho_O.*(O_hat - (v_TV_O - u_TV_O)) + 2*mu*dReLU_O.*ReLU_O;
                    st_r = 2*(-1i*exp(-1i*r_hat)).*sum(2*conj(batch_A).*MyIFFT2(U.*batch_F.*O_hat.*reshape(batch_e, [1,1,length(batch_idx)])),3);
        ...
        shift_dist = norm([rows_shift+row_add_best,cols_shift+col_add_best]);

        % 結果の表示
        title_corrmap = sprintf('Correlation map (shift=%.2f pix.)',shift_dist);
        figure(30);
        ...
        %結果を保存
        save_dir = sprintf('./figure_U_検証/M2_%s_Usigma%.1f_N%d_K%d_noise%d/',array_name,Usigmas(idx_U),N,K,noiseLv);
        mkdir(save_dir);
        ...
        finishMessage = sprintf('Usigma=%.1f(%d/%d),位相バイアスseed(%d/%d)の結果を保存 (RMSE_o_best=%.4f,SSIM_o_best=%.4f,RMSE_r_best=%.4f, [row,col]_add=[%d,%d])',Usigmas(idx_U),idx_U,Us_len,seed,num_arrays,RMSE_o_best,SSIM_o_best,RMSE_r_best,row_add_best,col_add_best);
        disp(finishMessage);
        %SSIM,RMSE,shiftdistを保存
        RMSE_tmp_o(seed) = RMSE_o_best;
        SSIM_tmp_o(seed) = SSIM_o_best;
        RMSE_tmp_r(seed) = RMSE_r_best;
        shift_dist_tmp(seed) = shift_dist;

              
    end %アンテナ配置seed（num_arrays通り）を切り替えてのループ終了

    RMSEs_o(idx_U) = mean(RMSE_tmp_o);
    stds_rmse_o(idx_U) = std(RMSE_tmp_o);
    SSIMs_o(idx_U) = mean(SSIM_tmp_o);
    stds_ssim_o(idx_U) = std(SSIM_tmp_o);
    RMSEs_r(idx_U) = mean(RMSE_tmp_r);
    stds_r(idx_U) = std(RMSE_tmp_r);
    shift_dists(idx_U) = mean(shift_dist_tmp);
    stds_shiftdist(idx_U) = std(shift_dist_tmp);

    clearvars phi A
end

RMSE_path = './figure_U_検証/RMSE/';
mkdir(RMSE_path);

figure(1000);
subplot(2,2,1)
errorbar(Usigmas,RMSEs_o,stds_rmse_o,'LineWidth',1);
title('RMSE of object');
xlabel('Sigma of antenna FFP');
ylabel('Reconstruction RMSE');

subplot(2,2,2)
errorbar(Usigmas,SSIMs_o,stds_ssim_o,'LineWidth',1);
title('SSIM of object');
xlabel('Sigma of antenna FFP');
ylabel('Reconstruction SSIM');

subplot(2,2,3)
errorbar(Usigmas,RMSEs_r,stds_r,'LineWidth',1);
title('RMSE of phase bias');
xlabel('Sigma of antenna FFP');
ylabel('Reconstruction RMSE');

subplot(2,2,4)
errorbar(Usigmas,shift_dists,stds_shiftdist,'LineWidth',1);
title('Shift distance');
xlabel('Sigma of antenna FFP');
ylabel('Shift distance');

%RMSEのグラフを保存
RMSE_fig = sprintf('%sM2_%s_%s_N%d_sup%d_noise%d_numE%d.fig',RMSE_path,obj_name,array_name,N,sup_size,noiseLv,num_arrays);
RMSE_png = sprintf('%sM2_%s_%s_N%d_sup%d_noise%d_numE%d.png',RMSE_path,obj_name,array_name,N,sup_size,noiseLv,num_arrays);
savefig(RMSE_fig);
