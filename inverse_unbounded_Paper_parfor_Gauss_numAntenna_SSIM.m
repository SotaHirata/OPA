...
SSIMs_o = zeros(num_antennas_len, 1);
stds_ssim_o = zeros(num_antennas_len, 1);
...
for idx_antenna = 1:num_antennas_len     %アンテナ数を切り替えてループ
    ...
    stds_rmse_o(idx_antenna) = std(RMSE_tmp_o);
    SSIMs_o(idx_antenna) = mean(SSIM_tmp_o);
    stds_ssim_o(idx_antenna) = std(SSIM_tmp_o);
...
figure(1000);
subplot(1,3,1)
errorbar(num_antennas,RMSEs_o,stds_rmse_o);
title('RMSE of object');
xlabel('Number of antennas');
ylabel('Reconstruction RMSE');

subplot(1,3,2)
errorbar(num_antennas,SSIMs_o,stds_ssim_o);
